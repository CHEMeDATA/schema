		...
		if (event.selection) {
			...
			if (this.keyTypes[key] === "string") {
				...
				const selectedBands = this.y[key]
					...
					.filter(
						(d) =>
							this.y[key](d) - this.y[key].bandwidth()/2 > y0 &&
							this.y[key](d) + this.y[key].bandwidth()/2< y1
					);
				console.log("this.y[key].bandwidth(): ", this.y[key].bandwidth());
				...
				let minDiff = Math.min(...selectedBands) ;
								console.log("[minDiff, maxDiff] ", [minDiff, maxDiff]);
