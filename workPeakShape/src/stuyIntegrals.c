...
int main() {
    ...
    int num_intervals = 100000; // Number of intervals for approximation
    ...
    printf("\n");
        ...
        printf("The integral of the Lorentzian/Gaussian  function from %.2f to %.2f \n", lower_limit, upper_limit);

    for (double balance = 0.0 ; balance < 1.01; balance += 0.1) {
        double integral_result = integral_approximationLG(mu, FWHM, lower_limit, upper_limit, num_intervals, balance);
         double aprox = (1.06446701943125+ 0.505829307404979 * balance) ;
                printf("%.3f, %.16f, %.16f, %.16f\n", balance, integral_result, aprox, aprox-integral_result);

    }
    printf("\n");
        printf("The integral of the generalized Lorentizan function from %.2f to %.2f \n", lower_limit, upper_limit );

    for (double balance = -1 ; balance < 2.01; balance += 0.1) {
        double integral_result = integral_approximationGeneralized(mu, FWHM, lower_limit, upper_limit, num_intervals, balance);
      double aprox = (1.57029632683931 - 0.210196803679899 * balance );
        printf("%.3f, %.16f, %.16f, %.16f\n", balance, integral_result, aprox, aprox-integral_result);
        //printf("1 / (2 * sqrt( 2 * log(2))) =  %.20f \n", 1.0 / (2 * sqrt( 2 * log(2))));
    }
    printf("\n");
